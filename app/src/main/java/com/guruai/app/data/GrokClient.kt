...
class GrokClient(private val apiKey: String) {
    ...
    suspend fun chat(userMessage: String, history: List<Pair<String, String>> = emptyList()): String {
        if (apiKey.isBlank()) {
            return "xAI (Grok) API key missing. Open Master Settings (password protected) and add your key."
        }
        ...
        while (attempt < maxAttempts) {
            ...
            if (attempt >= maxAttempts) {
                return "Grok is getting a lot of requests right now (rate limit). Please try again in a minute."
            }
            delay(delayMs)
            delayMs *= 2
        }
    ...
    private fun callOnce(userMessage: String, history: List<Pair<String, String>>): Result {
        ...
        client.newCall(request).execute().use { resp ->
            ...
            if (!resp.isSuccessful) {
                return Result("Grok error ${resp.code}: ${raw.take(200)}", resp.code)
            }
            val text = try {
